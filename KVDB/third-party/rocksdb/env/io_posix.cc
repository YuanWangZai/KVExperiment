...
static bvar::LatencyRecorder g_rocksdb_fs_posix_io_read("rocksdb_fs_posix_io_read");
static bvar::LatencyRecorder g_rocksdb_fs_posix_io_write("rocksdb_fs_posix_io_write");
static bvar::LatencyRecorder g_rocksdb_fs_posix_io_pread("rocksdb_fs_posix_io_pread");
static bvar::LatencyRecorder g_rocksdb_fs_posix_io_pwrite("rocksdb_fs_posix_io_pwrite");
static bvar::LatencyRecorder g_rocksdb_fs_posix_io_fread_unlocked("rocksdb_fs_posix_io_fread_unlocked");

static bvar::Status<uint64_t> g_rocksdb_fs_posix_io_read_byte("rocksdb_fs_posix_io_read_byte", 0);
static bvar::Status<uint64_t> g_rocksdb_fs_posix_io_write_byte("rocksdb_fs_posix_io_write_byte", 0);
static bvar::Status<uint64_t> g_rocksdb_fs_posix_io_pread_byte("rocksdb_fs_posix_io_pread_byte", 0);
static bvar::Status<uint64_t> g_rocksdb_fs_posix_io_pwrite_byte("rocksdb_fs_posix_io_pwrite_byte", 0);
static bvar::Status<uint64_t> g_rocksdb_fs_posix_io_fread_unlocked_byte("rocksdb_fs_posix_io_fread_unlocked_byte", 0);
...
bool PosixWrite(int fd, const char* buf, size_t nbyte) {
  ...
  while (left != 0) {
    ...
    g_rocksdb_fs_posix_io_write << (butil::cpuwide_time_us() - start_time);
    g_rocksdb_fs_posix_io_write_byte.set_value(done);
    if (done < 0) {
      if (errno == EINTR) {
        continue;
      }
...
bool PosixPositionedWrite(int fd, const char* buf, size_t nbyte, off_t offset) {
  ...
  while (left != 0) {
    ...
    g_rocksdb_fs_posix_io_pwrite << (butil::cpuwide_time_us() - start_time);
    g_rocksdb_fs_posix_io_pwrite_byte.set_value(done);
    if (done < 0) {
      if (errno == EINTR) {
        continue;
      }
...
IOStatus PosixSequentialFile::Read(size_t n, const IOOptions& /*opts*/,
  ...
  do {
    ...
    g_rocksdb_fs_posix_io_fread_unlocked << (butil::cpuwide_time_us() - start_time);
    g_rocksdb_fs_posix_io_fread_unlocked_byte.set_value(r);
...
IOStatus PosixSequentialFile::PositionedRead(uint64_t offset, size_t n,
  ...
  while (left > 0) {
    ...
    g_rocksdb_fs_posix_io_pread << (butil::cpuwide_time_us() - start_time);
	  g_rocksdb_fs_posix_io_pread_byte.set_value(r);
    if (r <= 0) {
      if (r == -1 && errno == EINTR) {
        continue;
      }
...
IOStatus PosixRandomRWFile::Read(uint64_t offset, size_t n,
  ...
  while (left > 0) {
    ...
    g_rocksdb_fs_posix_io_pread << (butil::cpuwide_time_us() - start_time);
	  g_rocksdb_fs_posix_io_pread_byte.set_value(done);
    if (done < 0) {
