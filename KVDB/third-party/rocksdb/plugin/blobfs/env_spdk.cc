...
#define CORE_MASK "0x3" // 原生blobfs是2个核，其中一个核心注册cache_reclaim_poller
static uint32_t CORE_NUM = 2; // 修改CORE_MASK的时候也要对CORE_NUM进行修改

// #define	FIXED_FILE_CHANNEL
namespace rocksdb
{
static bvar::LatencyRecorder g_rocksdb_fs_spdk_io_read("rocksdb_fs_spdk_io_read");
static bvar::LatencyRecorder g_rocksdb_fs_spdk_io_write("rocksdb_fs_spdk_io_write");

static bvar::Status<uint64_t> g_rocksdb_fs_spdk_io_read_byte("rocksdb_fs_spdk_io_read_byte", 0);
static bvar::Status<uint64_t> g_rocksdb_fs_spdk_io_write_byte("rocksdb_fs_spdk_io_write_byte", 0);
...
#ifdef FIXED_FILE_CHANNEL 
	ret = spdk_file_read_directly_channel(mFile, g_sync_args.channel, scratch, mOffset, n, core);
#else
	ret = spdk_file_read_directly(mFile, g_sync_args.channel, scratch, mOffset, n);
#endif
	g_rocksdb_fs_spdk_io_read << (butil::cpuwide_time_us() - start_time);
	g_rocksdb_fs_spdk_io_read_byte.set_value(n);
	if (ret >= 0) {
		mOffset += ret;
...
SpdkRandomAccessFile::Read(uint64_t offset, size_t n, Slice *result, char *scratch) const
{
	...
	int64_t start_time = butil::cpuwide_time_us();
#ifdef FIXED_FILE_CHANNEL 
	rc = spdk_file_read_directly_channel(mFile, g_sync_args.channel, scratch, offset, n, core);
#else
...
SpdkWritableFile::Append(const Slice &data)
{
	...
	int64_t start_time = butil::cpuwide_time_us();
#ifdef FIXED_FILE_CHANNEL
	rc = spdk_file_write_directly_channel(mFile, g_sync_args.channel, (void *)data.data(), mSize, data.size(), core);
#else
	rc = spdk_file_write_directly(mFile, g_sync_args.channel, (void *)data.data(), mSize, data.size());
#endif
	g_rocksdb_fs_spdk_io_write << (butil::cpuwide_time_us() - start_time);
	g_rocksdb_fs_spdk_io_write_byte.set_value(data.size());
	if (rc >= 0) {
		mSize += data.size();
		return Status::OK();
	} else {
...
rocksdb_shutdown()
{
	std::cout << "SPDK Env Shutdown" << std::endl;
}
