...
uint64_t NewSequentialFile_times = 0;
uint64_t NewRandomAccessFile_times = 0;
uint64_t NewWritableFile_times = 0;
uint64_t ReuseWritableFile_times = 0;
uint64_t NewDirectory_times = 0;
uint64_t DeleteFile_times = 0;
uint64_t FileExists_times = 0;
uint64_t RenameFile_times = 0;
uint64_t GetFileSize_times = 0;
uint64_t LockFile_times = 0;
uint64_t UnlockFile_times = 0;
uint64_t GetChildren_times = 0;

uint64_t spdk_file_read_times = 0;
uint64_t spdk_file_truncate_times = 0;
uint64_t spdk_file_sync_times = 0;
uint64_t spdk_file_write_times = 0;
uint64_t spdk_fs_open_file_times = 0;
uint64_t spdk_fs_file_stat_times = 0;
uint64_t spdk_fs_rename_file_times = 0;
uint64_t spdk_fs_delete_file_times = 0;
uint64_t spdk_file_get_name_times = 0;
uint64_t spdk_file_close_times = 0;
uint64_t spdk_file_set_priority_times = 0;
uint64_t spdk_file_get_id_times = 0;

void printTimes() {
    std::cout << "NewSequentialFile_times: " << NewSequentialFile_times << std::endl;
    std::cout << "NewRandomAccessFile_times: " << NewRandomAccessFile_times << std::endl;
    std::cout << "NewWritableFile_times: " << NewWritableFile_times << std::endl;
    std::cout << "ReuseWritableFile_times: " << ReuseWritableFile_times << std::endl;
    std::cout << "NewDirectory_times: " << NewDirectory_times << std::endl;
	std::cout << "DeleteFile_times: " << DeleteFile_times << std::endl;
    std::cout << "FileExists_times: " << FileExists_times << std::endl;
    std::cout << "RenameFile_times: " << RenameFile_times << std::endl;
    std::cout << "GetFileSize_times: " << GetFileSize_times << std::endl;
    std::cout << "LockFile_times: " << LockFile_times << std::endl;
    std::cout << "UnlockFile_times: " << UnlockFile_times << std::endl;
    std::cout << "GetChildren_times: " << GetChildren_times << std::endl << std::endl;

    std::cout << "spdk_file_read_times: " << spdk_file_read_times << std::endl;
    std::cout << "spdk_file_truncate_times: " << spdk_file_truncate_times << std::endl;
    std::cout << "spdk_file_sync_times: " << spdk_file_sync_times << std::endl;
    std::cout << "spdk_file_write_times: " << spdk_file_write_times << std::endl;
    std::cout << "spdk_fs_open_file_times: " << spdk_fs_open_file_times << std::endl;
	std::cout << "spdk_file_close_times: " << spdk_file_close_times << std::endl;
    std::cout << "spdk_fs_file_stat_times: " << spdk_fs_file_stat_times << std::endl;
    std::cout << "spdk_fs_rename_file_times: " << spdk_fs_rename_file_times << std::endl;
    std::cout << "spdk_fs_delete_file_times: " << spdk_fs_delete_file_times << std::endl;
    std::cout << "spdk_file_get_name_times: " << spdk_file_get_name_times << std::endl;
    std::cout << "spdk_file_set_priority_times: " << spdk_file_set_priority_times << std::endl;
    std::cout << "spdk_file_get_id_times: " << spdk_file_get_id_times << std::endl;
...
SpdkSequentialFile::Read(size_t n, Slice *result, char *scratch)
{
	...
	set_channel();
	ret = spdk_file_read_directly(mFile, g_sync_args.channel, scratch, mOffset, n);
	spdk_file_read_times++;
	if (ret >= 0) {
		mOffset += ret;
...
SpdkRandomAccessFile::Read(uint64_t offset, size_t n, Slice *result, char *scratch) const
{
	...
	set_channel();
	rc = spdk_file_read_directly(mFile, g_sync_args.channel, scratch, offset, n);
	spdk_file_read_times++;
	if (rc >= 0) {
...
SpdkWritableFile::Append(const Slice &data)
{
	...
	set_channel();
	rc = spdk_file_write_directly(mFile, g_sync_args.channel, (void *)data.data(), mSize, data.size());
	spdk_file_write_times++;
	if (rc >= 0) {
		mSize += data.size();
		return Status::OK();
	} else {
...
public:
	...
	struct JoinThreadsOnExit {
		explicit JoinThreadsOnExit(SpdkEnv& _deflt) : deflt(_deflt) {}
		~JoinThreadsOnExit() {
			delete &deflt;
		}
		SpdkEnv& deflt;
	...
	// 调用blobfs的spdk_fs_open_file方法
	// "/tmp/rocksdbtest-0/dbbench/CURRENT"
	virtual Status NewSequentialFile(const std::string &fname,
					 std::unique_ptr<SequentialFile> *result,
					 const EnvOptions &options) override
	{
		NewSequentialFile_times++;
	...
	// "/tmp/rocksdbtest-0/dbbench/000000.dbtmp"
	virtual Status NewWritableFile(const std::string &fname,
				       std::unique_ptr<WritableFile> *result,
				       const EnvOptions &options) override
	{
		NewWritableFile_times++;
		if (fname.compare(0, mDirectory.length(), mDirectory) == 0) {
			std::string name = sanitize_path(fname, mDirectory);
			struct spdk_file *file;
			int rc;
...
rocksdb_run(__attribute__((unused)) void *arg1)
{
	...
	printf("using bdev %s\n", g_bdev_name.c_str());
	// 从给定的 blobstore 块设备加载 blobstore 文件系统。
	// 是直接加载，所以需要先用mkfs进行创建
...
/** 退出清理应该由Env的析构函数调用__rocksdb_shutdown来做
 *  但是Ctrl+C退出的时候rocksdb_shutdown总是会被调用，而析构函数有时不会被执行
 *  为了获取统计指标，将printTimes写入rocksdb_shutdown中
 */
static void
__rocksdb_shutdown(void* ctx)
{
	if (g_fs != NULL) {
...
SpdkEnv::~SpdkEnv()
{
	...
	spdk_thread_send_critical_msg(spdk_thread_get_app_thread(), __rocksdb_shutdown);
		...
		const std::string &bdev, uint64_t cache_size_in_mb)
{
	try {
		...
		static SpdkEnv &spdk_env = *new SpdkEnv(base_env, dir, conf, bdev, cache_size_in_mb);
		if (g_fs != NULL) {
			static SpdkEnv::JoinThreadsOnExit thread_joiner(spdk_env);
			return &spdk_env;
		} else {
			delete &spdk_env;
			return NULL;
