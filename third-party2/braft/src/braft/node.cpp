...
static bvar::LatencyRecorder g_get_entry_by_read1_latency("raft_get_entry_by_read1");
static bvar::LatencyRecorder g_get_entry_by_read2_latency("raft_get_entry_by_read2");
...
butil::Status NodeImpl::read_committed_user_log(const int64_t index, UserLog* user_log) {
    ...
    int64_t now1 = butil::cpuwide_time_us();
    int64_t delta_time_us1 = 0;
    LogEntry* entry = _log_manager->get_entry(cur_index);
    delta_time_us1 = butil::cpuwide_time_us() - now1;
    g_get_entry_by_read1_latency << delta_time_us1;
    if (entry == NULL) {
    ...
    do {
        ...
        int64_t now2 = butil::cpuwide_time_us();
        int64_t delta_time_us2 = 0;
        entry = _log_manager->get_entry(cur_index);
        delta_time_us2 = butil::cpuwide_time_us() - now2;
        g_get_entry_by_read2_latency << delta_time_us2;
    } while (entry != NULL);
