...
static bvar::LatencyRecorder g_get_entry_from_mem_latency("raft_get_entry_from_mem");
static bvar::LatencyRecorder g_get_entry_from_log_latency("raft_get_entry_from_log");
...
LogEntry* LogManager::get_entry(const int64_t index) {
    ...
    int64_t mem_now = butil::cpuwide_time_us();
    int64_t mem_delta_time_us = 0;
    LogEntry* entry = get_entry_from_memory(index);
    mem_delta_time_us = butil::cpuwide_time_us() - mem_now;
    g_get_entry_from_mem_latency << mem_delta_time_us;
    ...
    g_get_entry_from_log_latency << delta_time_us;
